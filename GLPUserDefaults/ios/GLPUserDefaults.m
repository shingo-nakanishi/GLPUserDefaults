...
@implementation GLPUserDefaults

static id gLPUserDefaults = nil;
...
- (id)init
{
    ...
    if (self) {
        ...
        for(int i=0; i<propertiesNames.count; i++){
            ...
            class_addMethod([GLPUserDefaults class], setSel, setImp, "v@:@");
            ...
            class_addMethod([GLPUserDefaults class], getSel, getImp, "@@:");
...
+(instancetype)sharedManager{
    @synchronized(self) {
        if (gLPUserDefaults == nil) {
            gLPUserDefaults = [[self alloc] init];
        }
    }
    return gLPUserDefaults;
}
